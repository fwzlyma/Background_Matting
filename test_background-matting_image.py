...
class mat():
    ...
    def img_mat(self):
        ...
        args = Args()
        args.input_dir = 'sample_data/input'
        args.output_dir = 'sample_data/output'
        args.tranined_model = 'real-fixed-cam'
        # image mode
        args.target_back = 'sample_data/background/0001.png'
        # ******************************************
        self.model_main_dir = 'Models/' + args.tranined_model + '/'
        self.data_path = args.input_dir
        ...
        fo = glob.glob(self.model_main_dir + 'netG_epoch_*')
        model_name1 = fo[0]
        ...
        test_imgs = [f for f in os.listdir(self.data_path) if
                     os.path.isfile(os.path.join(self.data_path, f)) and f.endswith('_img.png')]
        test_imgs.sort()
        ...
        for i in range(0, len(test_imgs)):
            ...
            bgr_img = cv2.imread(os.path.join(self.data_path, filename))
            bgr_img = cv2.cvtColor(bgr_img, cv2.COLOR_BGR2RGB)
            rcnn = cv2.imread(os.path.join(self.data_path, filename.replace('_img', '_maskDL')), 0)

            if args.back is None:
                self.bg_im0 = cv2.imread(os.path.join(self.data_path, filename.replace('_img', '_back')))
                self.bg_im0 = cv2.cvtColor(self.bg_im0, cv2.COLOR_BGR2RGB)
            ...
            if args.video:
                ...
                for t in range(0, 4):
                    ...
                    bgr_img_mul = cv2.imread(os.path.join(self.data_path, file_tmp))
            ...
            bgr_img0 = bgr_img
            print(bgr_img0.shape[0])
            print(bgr_img0.shape[1])
            bbox = get_bbox(rcnn, R=bgr_img0.shape[0], C=bgr_img0.shape[1])
            print(bbox)
            crop_list = [bgr_img, self.bg_im0, rcnn, multi_fr_w]
            ...
            with torch.no_grad():
                img, bg, rcnn_al, multi_fr = Variable(img.cuda()), Variable(bg.cuda()), Variable(
                    rcnn_al.cuda()), Variable(
                    multi_fr.cuda())
            ...
            cv2.imwrite(result_path + '/' + filename.replace('_img', '_compose'),
